...
export default class AttackUpEffect extends PIXI.Container implements UpdateObject {
        public static readonly TextureFrameUpdateFrequency: number = 4;
        private elapsedFrameCount: number = 0;
        private sprite!: PIXI.Sprite;
        public static get resourceList(): string[] {
            return [Resource.Static.AttackUp];
        }
        ...
        constructor(onAnimationEnded: () => void) {
            ...
            this.sprite = new PIXI.Sprite(Resource.TextureFrame.AttackUp(1));
        ...
        public update(_delta: number): void {
            ...
            const frequency = AttackUpEffect.TextureFrameUpdateFrequency;
            if(this.elapsedFrameCount % frequency === 0) {
                const count = this.elapsedFrameCount / frequency;
                const index = Math.floor(count) + 1;
                if(index > Resource.MaxFrameIndex(Resource.Static.AttackUp)) {
                ...
                this.sprite.texture = Resource.TextureFrame.AttackUp(index);
            }
        }
    }